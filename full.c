//compatibility steps for different shell types
//gcc -o full.exe full.c -lpdcurses for windows target
//gcc -o full full.c -lncurses for linux target

#ifdef _WIN32
    #include <curses.h>    // use pdcurses on windows
    #include <windows.h>   // for sleep
#else
    #include <ncurses.h>   // use ncurses on unix-like systems
...
void save_grid_to_file(const char *filename) {
    ...
    clrtoeol();
    refresh();
    sleep(2000000); // pause a bit to let the user see the message
}
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (!paused) {
            ...
            sleep(100000);  // wait 0.1 seconds before updating again
        } else {
            draw_grid("simulation paused. press 'p' to resume, 'q' to quit.");
            sleep(100000);  // still wait a bit to reduce CPU usage
        }
    }
